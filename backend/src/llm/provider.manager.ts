...
import type { ConversationHistory } from "../types/conversation.types";

export class providerManager {
    private primaryProvider = groqProvider;
    private fallbackProvider = geminiProvider;

    async generateReply(conversationHistory: ConversationHistory, userMessage: string): Promise<string> {
        try {
            console.log("Using primary provider: Groq");
            const reply = await this.primaryProvider.generateReply(conversationHistory, userMessage);
            ...
            try {
                ...
                const reply = await this.fallbackProvider.generateReply(conversationHistory, userMessage);
                return reply;
